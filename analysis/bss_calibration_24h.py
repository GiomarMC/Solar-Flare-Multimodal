"""
Calibración probabilística y Brier Skill Score (BSS) a 24 h — SIN reentrenar.

Versión paralela de bss_calibration.py (48 h) para el horizonte de 24 h. Trabaja
sobre los logits ya guardados (outputs/logits/{model}_24h_k{fold}_{split}.npz),
...
τ_opt se deriva por barrido sobre el VAL (mismo criterio max-TSS con que se
eligió en el entrenamiento), de modo que el script es autocontenido y no depende
...
def run_model(model, lines):
    ...
    for k in FOLDS:
        ...
        taus[k] = sweep_tau(sigmoid(v[0]), v[1], 0.01, 0.99, 0.01)[0]
        labT = t[1].astype(np.float64)

    pbar = float(np.mean(labT))
    lines.append(f"\n{'='*70}\n  {model.upper()} — 24 h   (tasa base test = {pbar:.4f})\n{'='*70}")
    lines.append(f"  {'Fold':<5}{'τ':>6}{'BS_raw':>9}{'BSS_raw':>9}{'BS_cal':>9}{'BSS_cal':>9}{'método':>11}{'TSS':>8}")

    raw_test_probs, cal_test_probs = [], []
    bss_cal_folds, tss_folds = [], []
    for k in sorted(test):
        ...
        lines.append(f"  k={k:<3}{taus[k]:>6.2f}{bs_raw:>9.4f}{bss_raw:>9.4f}{bs_cal:>9.4f}{bss_cal:>9.4f}{method:>11}{tss:>8.4f}")
        raw_test_probs.append(p_t_raw); cal_test_probs.append(p_t_cal)
        bss_cal_folds.append(bss_cal); tss_folds.append(tss)
    ...
    lines.append(f"  {'-'*70}")
    lines.append(f"  Por fold:  BSS_cal = {np.mean(bss_cal_folds):.4f} ± {np.std(bss_cal_folds, ddof=1):.4f}   "
                 f"TSS = {np.mean(tss_folds):.4f} ± {np.std(tss_folds, ddof=1):.4f}")
...
def main():
    lines = ["Calibración probabilística y BSS — 24 h (sin reentrenar)"]
