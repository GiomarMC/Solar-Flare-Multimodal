"""
Entrenamiento standalone de la LSTM sobre parámetros SHARP v3 (17 params).
...
Usage:
    python scripts/train_lstm_standalone.py --config configs/lstm_standalone_k3.yaml
    python scripts/train_lstm_standalone.py --config configs/lstm_standalone_k3.yaml --smoke 5
"""
...
class SharpOnlyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tab = _log_transform(np.stack(all_params, axis=0))  # [T, 17]
        return (
...
def main():
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename='sfmm_lstm_standalone-{epoch:03d}-{val_tss:.4f}',
        monitor='val_tss',
        mode='max',
        save_top_k=3,
    )
    ...
    logger = CSVLogger(cfg['output']['log_dir'], name='sfmm_lstm_standalone')
