"""
Ablación de arquitectura de la BiLSTM standalone — 48 h (parametriza hidden y num_layers).
...
def build_or_load_cache(cfg, fold):
    ...
    path = os.path.join(CACHE_DIR, f"fold{fold}_48h.npz")
...
def main():
    ...
    H, k = 48, args.fold
    tag = f"h{args.hidden}_l{args.num_layers}_48h_k{k}"
    ...
    cfg['output']['checkpoint_dir'] = f"outputs/checkpoints_lstm_abl_{tag}"
    cfg['output']['log_dir'] = f"outputs/logs_lstm_abl_{tag}"
    ...
    np.savez_compressed(os.path.join(out_dir, f"lstm_abl_{tag}_val.npz"), logits=vl, labels=vy)
    np.savez_compressed(os.path.join(out_dir, f"lstm_abl_{tag}_test.npz"), logits=tl, labels=ty)
