"""
Dataset temporal v8 — 16 parámetros SHARP: los 17 de v3 sin MEANGBZ.
...
idéntica en los 5 folds y en ambos horizontes salvo por un caso en la frontera:
    ...
    MEANGBZ   48h: d = 0.634 (dataset completo)  vs  0.494-0.580 (por fold)
              24h: d = 0.589 (dataset completo)  vs  0.484-0.558 (por fold)

Es decir, un criterio estrictamente interno al fold también lo habría descartado.
Este dataset entrena la rama física sin él para comprobar que el resultado no
depende de ese parámetro (su importancia por permutación es ~0).

NO confundir con v6 (también 16 params, pero los 16 vectoriales: quita los 5
derivados del LoS y reincorpora los 4 de baja d). Aquí la ÚNICA diferencia
respecto a v3 es MEANGBZ; todo lo demás (arquitectura, splits, protocolo) es igual.
"""
...
# v8: los 17 de v3 menos MEANGBZ (índice 8). Siguen fuera los 4 que v3 descartaba
#     (5=MEANGBH, 9=MEANJZH, 10=MEANJZD, 13=MEANALP).
_KEEP_IDX = [0, 1, 2, 3, 4, 6, 7, 11, 12, 14, 15, 16, 17, 18, 19, 20]
...
def load_para_flare(para_path: str):
    """Returns dict: stem_name -> np.array(16,) — los 17 de v3 sin MEANGBZ."""
    mapping = {}
...
class MultimodalTemporalDataset(Dataset):
    """
    Returns (X_vid, X_tab, label) para SF-MM con 16 parámetros SHARP (v3 sin MEANGBZ).

    X_vid : [3, T, H, W]   — T=16 magnetogramas, replicados a 3 canales
    X_tab : [T, 16]         — T=16 timestamps × 16 SHARP (log-transformados)
    label : escalar int     — 0/1

    v8: 16 params (v3 sin MEANGBZ), sin hflip/vflip.
    """
