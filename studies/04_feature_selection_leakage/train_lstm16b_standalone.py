"""
Entrenamiento standalone de la BiLSTM sobre 16 parámetros SHARP (v8): los 17 del
paper sin MEANGBZ.

Copia paralela de train_lstm12_standalone.py — misma arquitectura, mismo protocolo,
mismos splits e hiperparámetros que el BiLSTM-17 del paper. La ÚNICA diferencia con
el BiLSTM-17 es el dataset (v8 en lugar de v3).

Objetivo: responder al Revisor #1 de SIMBig. Al recalcular la d de Cohen solo con
los folds de entrenamiento, MEANGBZ cae por debajo del umbral de 0.6 (0.49-0.58
por fold frente a 0.63 en el dataset completo), de modo que un criterio sin fuga
también lo habría descartado. Si el TSS no cambia, la preselección no afecta a los
...
class SharpOnlyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tab = _log_transform(np.stack(all_params, axis=0))  # [T, 16]
        return (
...
def main():
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename='sfmm_lstm16b_standalone-{epoch:03d}-{val_tss:.4f}',
        monitor='val_tss',
        mode='max',
        save_top_k=3,
    )
    ...
    logger = CSVLogger(cfg['output']['log_dir'], name='sfmm_lstm16b_standalone')
