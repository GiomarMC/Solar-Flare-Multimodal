"""
Evaluación de BiLSTM-16b standalone y guardado de logits.

Copia paralela de save_logits_lstm12.py para la variante de 16 parámetros SHARP
(dataset_temporal_v8: los 17 del paper sin MEANGBZ). Escribe en
outputs/logits/lstm16b_*.npz para no pisar los logits de otras variantes.

NO confundir con save_logits_lstm16.py (v6: los 16 VECTORIALES, otro conjunto).
"""
...
from train_lstm16b_standalone import (
...
def run_fold(base_dir: str, horizon: int, fold: int, device: torch.device):
    ...
    cfg_path  = os.path.join(_AQUI, "configs", f"lstm16b_cv_{horizon}h_k{fold}.yaml")
    ckpt_dir  = f"outputs/checkpoints_lstm16b_cv_{horizon}h_k{fold}"
    ...
    for split, logits, labels in [
        ...
        out = f"outputs/logits/lstm16b_{horizon}h_k{fold}_{split}.npz"
...
def main():
    ...
    sample_cfg = os.path.join(_AQUI, "configs", "lstm16b_cv_24h_k0.yaml")
    ...
    for h in horizons:
        for k in folds:
            print(f"── BiLSTM-16b {h}h k={k} ──────────────────────────")
            run_fold(base_dir, h, k, device)
            print()

    print("Logits BiLSTM-16b guardados en outputs/logits/lstm16b_*.npz")
