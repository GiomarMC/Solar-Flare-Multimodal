"""
Dataset temporal v7 — 12 parámetros SHARP: los 17 de v3 sin los 5 derivados del LoS.

Ablación PURA de los parámetros "hijos" de la imagen. Respecto a v3 (17 params,
el modelo del paper) la ÚNICA diferencia es que se eliminan los 5 que se calculan
sobre el mismo magnetograma de línea de visión (LoS) que ve la rama visual:
    USFLUXL, MEANGBL, R_VALUE   (Bobra et al. 2021, §3.2)
    AREA_ACR, NACR              (keywords HARP, Bobra et al. 2014, Tabla A.8)
Quedan los 12 vectoriales de v3 (Bobra et al. 2014, Tabla 3).

A diferencia de v6 (16 params), NO reincorpora MEANGBH, MEANJZH, MEANJZD, MEANALP:
v6 mezclaba dos cambios (quitar hijos + añadir esos 4) y no permitía atribuir el
efecto. Todo lo demás (arquitectura, splits, protocolo) es idéntico a v3.
"""
...
# v7: los 17 de v3 menos los 5 derivados del campo LoS (4=USFLUXL, 6=MEANGBL, 17=AREA_ACR,
#     18=NACR, 20=R_VALUE). NO se reincorporan los 4 que v3 descartaba (5, 9, 10, 13).
_KEEP_IDX = [0, 1, 2, 3, 7, 8, 11, 12, 14, 15, 16, 19]

SHARP_PARAMS = [_SHARP_21_ALL[i] for i in _KEEP_IDX]
N_SHARP = len(SHARP_PARAMS)  # 12
...
def load_para_flare(para_path: str):
    """Returns dict: stem_name -> np.array(12,) — los 12 vectoriales de v3."""
    mapping = {}
