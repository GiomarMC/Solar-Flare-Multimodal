"""
Evaluación de BiLSTM-21 standalone y guardado de logits.

Copia paralela de save_logits_lstm.py para la variante de 21 parámetros SHARP
(dataset_temporal_v5). Escribe en outputs/logits/lstm21_*.npz para no pisar los
logits de 17 params.
...
def run_fold(base_dir: str, horizon: int, fold: int, device: torch.device):
    ...
    for split, logits, labels in [
        ...
        out = f"outputs/logits/lstm21_{horizon}h_k{fold}_{split}.npz"
...
def main():
    ...
    for h in horizons:
        for k in folds:
            print(f"── BiLSTM-21 {h}h k={k} ──────────────────────────")
            run_fold(base_dir, h, k, device)
            print()

    print("Logits BiLSTM-21 guardados en outputs/logits/lstm21_*.npz")
