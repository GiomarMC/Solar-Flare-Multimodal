"""
Evaluación de BiLSTM-16 standalone y guardado de logits.

Copia paralela de save_logits_lstm16.py para la variante de 16 parámetros SHARP
vectoriales (dataset_temporal_v6). Escribe en outputs/logits/lstm16_*.npz para no
pisar los logits de 17 ni de 21 params.

Itera sobre los 5 folds ya entrenados para 24h y 48h.
Puede correrse en CPU (no necesita GPU).

Usage:
    python studies/03_los_derived_ablation/save_logits_lstm16.py
    python studies/03_los_derived_ablation/save_logits_lstm16.py --horizon 24
    python studies/03_los_derived_ablation/save_logits_lstm16.py --horizon 48 --fold 3
"""
...
from train_lstm16_standalone import (
...
def run_fold(base_dir: str, horizon: int, fold: int, device: torch.device):
    ...
    cfg_path  = os.path.join(_AQUI, "configs", f"lstm16_cv_{horizon}h_k{fold}.yaml")
    ckpt_dir  = f"outputs/checkpoints_lstm16_cv_{horizon}h_k{fold}"
    ...
    for split, logits, labels in [
        ...
        out = f"outputs/logits/lstm16_{horizon}h_k{fold}_{split}.npz"
...
def main():
    ...
    sample_cfg = os.path.join(_AQUI, "configs", "lstm16_cv_24h_k0.yaml")
    ...
    for h in horizons:
        for k in folds:
            print(f"── BiLSTM-16 {h}h k={k} ──────────────────────────")
            run_fold(base_dir, h, k, device)
            print()

    print("Logits BiLSTM-16 guardados en outputs/logits/lstm16_*.npz")
