"""
Entrenamiento standalone de la BiLSTM sobre 12 parámetros SHARP (v7): los 17 del
paper sin los 5 derivados del magnetograma LoS (USFLUXL, MEANGBL, R_VALUE,
AREA_ACR, NACR).

Copia paralela de train_lstm16_standalone.py — misma arquitectura, mismo protocolo,
mismos splits e hiperparámetros que el BiLSTM-17 del paper. La ÚNICA diferencia
con el BiLSTM-17 es el dataset (v7 en lugar de v3): ablación pura de los "hijos".

Usage:
    python studies/03_los_derived_ablation/train_lstm12_standalone.py --config studies/03_los_derived_ablation/configs/lstm12_cv_48h_k3.yaml
    python studies/03_los_derived_ablation/train_lstm12_standalone.py --config studies/03_los_derived_ablation/configs/lstm12_cv_48h_k3.yaml --smoke 5
"""
...
from dataset_temporal_v7 import (
...
class SharpOnlyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tab = _log_transform(np.stack(all_params, axis=0))  # [T, 12]
        return (
...
def main():
    ...
    parser.add_argument('--config', default=os.path.join(_AQUI, 'configs', 'lstm12_cv_48h_k3.yaml'))
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename='sfmm_lstm12_standalone-{epoch:03d}-{val_tss:.4f}',
        monitor='val_tss',
        mode='max',
        save_top_k=3,
    )
    ...
    logger = CSVLogger(cfg['output']['log_dir'], name='sfmm_lstm12_standalone')
