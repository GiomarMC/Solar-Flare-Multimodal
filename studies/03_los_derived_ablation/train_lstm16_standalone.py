"""
Entrenamiento standalone de la BiLSTM sobre los 16 parámetros SHARP vectoriales (v6).

Copia paralela de train_lstm21_standalone.py (21 params) — misma arquitectura,
mismo protocolo, mismos splits. La ÚNICA diferencia es el dataset: v6 en lugar
de v5, es decir sin los 5 parámetros derivados del magnetograma LoS
(USFLUXL, MEANGBL, R_VALUE, AREA_ACR, NACR).

Objetivo: comprobar si el techo estructural de la fusión se debe a que parte de
la rama física se deriva del mismo dato que ve la rama visual ("padre-hijo"),
...
def main():
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename='sfmm_lstm16_standalone-{epoch:03d}-{val_tss:.4f}',
        monitor='val_tss',
        mode='max',
        save_top_k=3,
    )
    ...
    logger = CSVLogger(cfg['output']['log_dir'], name='sfmm_lstm16_standalone')
