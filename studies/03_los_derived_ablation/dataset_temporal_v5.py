"""
Dataset temporal v5 — los 21 parámetros SHARP completos, sin hflip ni vflip.

Cambios respecto a v3 (dataset_temporal_v3.py):
  - REINCORPORA los 4 parámetros que v3 elimina por baja señal discriminativa
    univariada (MEANGBH d=0.14, MEANJZH d=0.11, MEANJZD d=0.53, MEANALP d=0.17).
  - Motivo: esos 4 son justamente los MENOS recuperables desde el magnetograma
    LOS que ve la rama visual (|rho| con agregados triviales de la imagen =
...
# v5: se conservan los 21 (v3 eliminaba 5=MEANGBH, 9=MEANJZH, 10=MEANJZD, 13=MEANALP)
_KEEP_IDX = list(range(21))

SHARP_PARAMS = [_SHARP_21_ALL[i] for i in _KEEP_IDX]
N_SHARP = len(SHARP_PARAMS)  # 21
...
def load_para_flare(para_path: str):
    """Returns dict: stem_name -> np.array(21,) — los 21 params completos."""
    mapping = {}
